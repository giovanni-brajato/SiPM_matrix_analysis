...
for file in fileList:
    ...
    with open(file, 'r') as currentFile:
        ...
        if FDList[m]=='D':
            ...
            voltageLevel.append("".join(currFolders[6].split('_')[3:]))
...
for m in range(Ncurves):
    if (ChList[m]==3):
        plt.plot(np.asarray(v10dac_list[m]), d_counts_mean_list[m] / d_counts_mean_list[m].max() * 100, color=GlobalChannelcolors[m], label="CH"+str(ChList[m]) +","+voltageLevel[m], linewidth=2)
