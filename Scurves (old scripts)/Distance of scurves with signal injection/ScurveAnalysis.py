...
for m in range(Ncurves):
    tempDist=  np.maximum.accumulate(d_counts_mean_list[m] / d_counts_mean_list[m].max())
    tempDist = np.concatenate((np.zeros(v10dac_list[m][0]),tempDist/tempDist.max(),np.ones(1023- v10dac_list[m][-1])))*100
    v_eta.append(linear_extrap(eta_prompt,tempDist,np.arange(1024)))
...
newColors = distinctipy.get_colors(int(Ncurves*(Ncurves-1)/2),rng=1)
dist_mat = np.zeros((Ncurves,Ncurves,101))
plt.figure()
ic = 0
for i in range (Ncurves):
    for j in range(i):
        dist_mat[i,j,:] = np.sqrt((np.asarray(v_eta[i]) - np.asarray(v_eta[j]))**2)
        plt.plot(eta_prompt,dist_mat[i,j,:],label="CH"+str(ChList[i]) + " - CH" +str(ChList[j]),color=newColors[ic])
        ic +=1
plt.ylabel('Scurve distance DACu')
plt.xlabel('Trigger efficiency %')
plt.legend()
plt.title("Distance with signal injection: 50mV @-10dB")
plt.show()