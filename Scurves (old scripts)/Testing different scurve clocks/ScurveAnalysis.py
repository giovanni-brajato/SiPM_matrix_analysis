...
sm.set_array([])
for m in range(Nexp):
    ax1.plot(threshold_10bitDAC_units_d_count[m], raw_d_counts_new[m]/raw_d_counts_new[m].max(), color=cmap(norm(freq_list_number[m])), label=freq_list[m])
cbar = plt.colorbar(sm,ax=ax1, label="Clock frequency",ticks=freq_list_number, format=matplotlib.ticker.ScalarFormatter(),
                    shrink=1.0, fraction=0.1, pad=0)
cbar.set_ticklabels(freq_list)
ax1.legend()
ax1.set_ylabel('Normalized raw duration')
ax1.set_title("Duration counter on 1s window with different clocks")

for m in range(Nexp):
    ax2.plot(threshold_10bitDAC_units_d_count[m], prob_from_d_counts[m]/prob_from_d_counts[m].sum(), color=cmap(norm(freq_list_number[m])), label=freq_list[m])
cbar = plt.colorbar(sm, ax=ax2,label="Clock frequency",ticks=freq_list_number, format=matplotlib.ticker.ScalarFormatter(),
                    shrink=1.0, fraction=0.1, pad=0)
cbar.set_ticklabels(freq_list)
ax2.legend()
ax2.set_xlabel('Vth time DAC units')
ax2.set_ylabel("Noise probabiliy density ")
ax2.set_title("Estimated noise probability density from duration counter on 1s window with different clocks")
