...
winLength_s = np.asarray(clkPeriod)*np.asarray(winLength)
winLength_s_str = ["1s","100ms","10ms","1ms","100us","10us","1us"]

f, (ax1, ax2) = plt.subplots(2, 1, sharex=True)
cmap = matplotlib.colormaps["viridis"]
norm = matplotlib.colors.LogNorm(vmin=winLength_s.min(), vmax=winLength_s.max())
sm = matplotlib.cm.ScalarMappable(norm=norm,cmap=cmap)
sm.set_array([])
for m in range(Nexp):
    ax1.errorbar(threshold_10bitDAC_units_d_count[m], raw_d_counts_new[m]/raw_d_counts_new[m].max(),xerr=None,yerr=raw_d_counts_new_std[m]/raw_d_counts_new[m].max(), color=cmap(norm(winLength_s[m])), label=winLength_s_str[m])
cbar = plt.colorbar(sm,ax=ax1, label="Win size",ticks=winLength_s, format=matplotlib.ticker.ScalarFormatter(),
                    shrink=1.0, fraction=0.1, pad=0)
cbar.set_ticklabels(winLength_s_str)
ax1.legend()
ax1.set_ylabel('Normalized raw duration')
ax1.set_title("Duration counter 1GHz clock on different windows")

for m in range(Nexp):
    ax2.plot(threshold_10bitDAC_units_d_count[m], prob_from_d_counts[m]/prob_from_d_counts[m].sum(), color=cmap(norm(winLength_s[m])), label=winLength_s_str[m])
cbar = plt.colorbar(sm, ax=ax2,label="Win size",ticks=winLength_s, format=matplotlib.ticker.ScalarFormatter(),
                    shrink=1.0, fraction=0.1, pad=0)
cbar.set_ticklabels(winLength_s_str)
ax2.legend()
...
plt.figure()
for m in range(Nexp):
    plt.errorbar(threshold_10bitDAC_units_d_count[m], raw_d_counts_new[m],xerr=None,yerr=raw_d_counts_new_std[m], color=cmap(norm(winLength_s[m])), label=winLength_s_str[m])
cbar = plt.colorbar(sm, label="Win size",ticks=winLength_s, format=matplotlib.ticker.ScalarFormatter(),
                    shrink=1.0, fraction=0.1, pad=0)
cbar.set_ticklabels(winLength_s_str)
plt.legend()
plt.yscale('log')
...
plt.figure()
for m in range(Nexp):
    plt.errorbar(threshold_10bitDAC_units_d_count[m], raw_d_counts_new[m]/raw_d_counts_new[m].max()*100,xerr=None,yerr=raw_d_counts_new_std[m]/raw_d_counts_new[m].max()*100, color=cmap(norm(winLength_s[m])), label=winLength_s_str[m])
cbar = plt.colorbar(sm, label="Win size",ticks=winLength_s, format=matplotlib.ticker.ScalarFormatter(),
                    shrink=1.0, fraction=0.1, pad=0)
cbar.set_ticklabels(winLength_s_str)
plt.legend()
...
sm.set_array([])
for m in range(Nexp):
    plt.plot(threshold_10bitDAC_units_f_count[m], raw_f_counts_new[m]/raw_f_counts_new[m].max(), color=cmap(norm(winLength_s[m])), label=winLength_s_str[m])
cbar = plt.colorbar(sm,label="Win size",ticks=winLength_s, format=matplotlib.ticker.ScalarFormatter(),
                    shrink=1.0, fraction=0.1, pad=0)
cbar.set_ticklabels(winLength_s_str)
plt.legend()
...
sm.set_array([])
for m in range(Nexp):
    plt.plot(threshold_10bitDAC_units_f_count[m], raw_f_counts_new[m]/winLength_s[m], color=cmap(norm(winLength_s[m])), label=winLength_s_str[m])
cbar = plt.colorbar(sm, label="Win size",ticks=winLength_s, format=matplotlib.ticker.ScalarFormatter(),
                    shrink=1.0, fraction=0.1, pad=0)
cbar.set_ticklabels(winLength_s_str)
plt.legend()
