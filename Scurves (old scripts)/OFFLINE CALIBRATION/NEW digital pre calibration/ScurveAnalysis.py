...
for file in fileList:
    ...
    with open(file, 'r') as currentFile:
        ...
        if FDList[m]=='D':
            ...
            if data.values[-1, 3:] == 0:
                raw_dcount_mean = np.cumsum(data.values[:, 3:].mean(1))
            else:
                raw_dcount_mean = np.maximum.accumulate(data.values[:, 3:].mean(1))
            ...
            final_dcount_mean[unavail_values_mask & (np.arange(1024) > raw_v10dac[-1])] = max(raw_dcount_mean)
...
mu_per_asic = [[[] for a in range(4)] for c in range(32)]
sigma_per_asic = [[[] for a in range(4)] for c in range(32)]
...
for m in range(Ncurves):
    ...
    # fitting a gaussian for each one of them
    xdata = v10dac_list[m]
    ydata = prob_mean_list[m]
    max_y = max(ydata)
    max_loc_x = np.squeeze(xdata[ydata==max_y])
    parameters, covariance = curve_fit(gauss, xdata, ydata, p0=[max_y,max_loc_x,1])
    mu_per_asic[ChList[m]][Asic_List[m] - 1] = parameters[1]
    sigma_per_asic[ChList[m]][Asic_List[m] - 1] = parameters[2]
...
fig, axs = plt.subplots(nrows=1,ncols=NAsics,sharey=True,sharex=True)
for a in range(NAsics):
    axs[a].errorbar(np.arange(32),np.asarray(mu_per_asic)[:,a],xerr=None,yerr=np.asarray(sigma_per_asic)[:,a],capsize=3)
    axs[a].set_xlabel('Channel #')
    axs[a].set_ylabel('Threshold DACu')
    axs[a].set_title('Scurves mu/sigma ASIC '+str(a+1))
...
for a in range(NAsics):
   ...
   """
    ...
    # better solution
    x_better = np.asarray(mu_per_asic)[:,a]*c10/c6 - v6_eta
    x_better = np.round(x_better - x_better.min())
    x_better = x_better.max() - x_better
    x_better[x_better < 0] = 0
    x_better[x_better > 63] = 63

    print(str(eta_obj) + "% 6bit DAC calibration values - better solution on gaussian fitting")
    for c in range(N):
        if np.isnan(x_better[c]):
            print("ASIC_" + str(a + 1) + " sixbDAC_ch" + str(c) + "  " + "0")
            x_better[c] = 0
        else:
            print("ASIC_" + str(a + 1) + " sixbDAC_ch" + str(c) + "  " + str(int(x_better[c])))
    v6_sol[:, a] = x_better
...
Cliquez sur un mot pour en savoir plus à son sujet ou voir d'autres traductions possibles.
...
global_sol_offset = np.zeros((32,4))
if (performGlobalCalibration):
        ...
        for dv6_2 in range(len(Nshift_list[1])):
            for dv6_3 in range(len(Nshift_list[2])):
                for dv6_4 in range(len(Nshift_list[3])):
                    vsol_temp = v6_sol + np.asarray([Nshift_list[0][dv6_1],Nshift_list[1][dv6_2],Nshift_list[2][dv6_3],Nshift_list[3][dv6_4]])
...
for m in range(Ncurves):
    if np.isnan(global_sol[ChList[m],Asic_List[m]-1]):
        print("ASIC_" + str(Asic_List[m]) + " sixbDAC_ch" + str(ChList[m]) + "  " + "???")
    else:
...
for m in range(Ncurves):
    ...
    minpos = np.argwhere(d_counts_mean_list[m] > 0)
    if len(minpos)>0:
        firstNonZeroIndex = np.max([minpos[0,0]-1,0])
    else:
        firstNonZeroIndex = 1023
