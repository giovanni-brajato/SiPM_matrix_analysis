...
for file in fileList:
    ...
    with open(file, 'r') as currentFile:
        ...
        if FDList[m]=='D':
            ...
            parameterList.append(currFolders[7])
...
vdac10_eta = np.zeros((len(eta_prompt), 32, 2))
for m in range(Ncurves):
    eff_curve = np.maximum.accumulate(d_counts_mean_list[m] / d_counts_mean_list[m].max() * 100)
    if parameterList[m] == 'Analog':
        vdac10_eta[:, ChList[m], 0] = linear_extrap(eta_prompt, eff_curve, v10dac_list[m])
    elif parameterList[m] == 'Digital':
        vdac10_eta[:, ChList[m], 1] = linear_extrap(eta_prompt, eff_curve, v10dac_list[m])

max_offset = np.zeros((len(eta_prompt),2))
...
plt.figure()
plt.plot(eta_prompt,max_offset[:,0],label='max scurve analog offset')
plt.plot(eta_prompt,max_offset[:,1],label='max scurve digital offset')
...
# globalChannelColors
GlobalChannelcolors = distinctipy.get_colors(32, rng =1)
...
for m in range(Ncurves):
        ...
        plt.plot(np.asarray(v10dac_list[m]), d_counts_mean_list[m] / d_counts_mean_list[m].max() * 100, color=GlobalChannelcolors[ChList[m]], label="CH"+str(ChList[m]) +","+parameterList[m], linewidth=2)
plt.yscale("log")
...
for m in range(Ncurves):
    if parameterList[m] == 'Analog':
        #plt.plot(np.asarray(v10dac_list[m]), d_counts_mean_list[m] / d_counts_mean_list[m].max() * 100, color=cmap(m/Ncurves), label="CH"+str(ChList[m]) +","+parameterList[m], linewidth=2)
        plt.plot(np.asarray(v10dac_list[m]), d_counts_mean_list[m] / d_counts_mean_list[m].max() * 100, color=GlobalChannelcolors[ChList[m]], label="CH"+str(ChList[m]) , linewidth=2)
plt.xlabel('Vth 10 bit DAC')
plt.ylabel('Trigger efficiency %')
plt.title("Analog")
...
plt.figure()
for m in range(Ncurves):
    if parameterList[m] == 'Digital':
        #plt.plot(np.asarray(v10dac_list[m]), d_counts_mean_list[m] / d_counts_mean_list[m].max() * 100, color=cmap(m/Ncurves), label="CH"+str(ChList[m]) , linewidth=2)
        plt.plot(np.asarray(v10dac_list[m]), d_counts_mean_list[m] / d_counts_mean_list[m].max() * 100, color=GlobalChannelcolors[ChList[m]], label="CH"+str(ChList[m]), linewidth=2)
plt.xlabel('Vth 10 bit DAC')
plt.ylabel('Trigger efficiency %')
plt.legend()
