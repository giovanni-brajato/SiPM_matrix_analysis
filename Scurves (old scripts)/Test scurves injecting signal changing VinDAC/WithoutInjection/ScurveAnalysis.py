...
# globalChannelColors
GlobalChannelcolors = distinctipy.get_colors(len(N_vindac))
cmap = matplotlib.cm.get_cmap('rainbow')

plt.figure()
for m in range(Ncurves):
    for v in range(len(N_vindac)):
        plt.plot(N_v10th, scurve_set[m,:,:,v].squeeze(), color=GlobalChannelcolors[v], label="CH"+str(ChList[m]) +", VinDac"+str(N_vindac[v]), linewidth=2)
plt.yscale("log")
