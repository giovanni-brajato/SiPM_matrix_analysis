...
class OcrToTableTool:
    ...
    def crop_each_bounding_box_and_ocr(self):
        ...
        for row in self.rows:
            for bounding_box in row:
                ...
                slice_name = "img_" + str(image_number) + ".jpg"
                image_slice_path =  os.getcwd()+"\\ocr_slices\\"
                if (not os.path.exists(image_slice_path)):
                    os.makedirs(image_slice_path)
                cv2.imwrite(image_slice_path + slice_name, cropped_image)
                results_from_ocr = self.get_result_from_tersseract(image_slice_path+slice_name)
    ...
    def store_process_image(self, file_name, image):
        path = "\\process_images\\ocr_table_tool\\"
        winpath = os.getcwd()+path
        if(not os.path.exists(winpath)):
            os.makedirs(winpath)
        cv2.imwrite(winpath+file_name, image)