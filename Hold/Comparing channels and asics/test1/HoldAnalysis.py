...
chargePedestal = np.asarray([Cpedestal1[3],Cpedestal1[12],Cpedestal0[4],Cpedestal0[9],Cpedestal0[15]])
...
for cin in range(4):
    for cf in range(4):
        for v in range(Nexp):
            axs[cin,cf].plot(holdVector_ns,VoltageCinCfHold_tensor[v,cin,cf,:], label=AsicChannelData[v] + " Vpp "+ f"{VppInData_mean[v]*1e3:.{2}f}"+ " mV",color=channelColors[v])
            axs[cin,cf].fill_between(holdVector_ns, VoltageCinCfHold_tensor[v,cin,cf,:] - VoltageCinCfHold_tensor_std[v,cin,cf,:], VoltageCinCfHold_tensor[v,cin,cf,:] + VoltageCinCfHold_tensor_std[v,cin,cf,:], color=channelColors[v], alpha=0.2)
            ...
            axs[cin,cf].plot(holdPeakPosition, ADCPeakValue,marker='x',color=channelColors[v])
        if cf== 3:
